...
def getResonmode(S,thr=3,search_factor=10,smooth=False,smooth_window=3):#search_factor=10,smooth_window=5to3
    ...
    if smooth:
        S=savitzky_golay(S,smooth_window,3)#3
    modes=list()
...
class Resonator(ZNB20,Qextractor):
    def __init__(self,f0=6e9,instr_name='ZNB20'):
        ZNB20.__init__(self,instr_name)
...
#        self.idx_broaden=0
        ...
        self.P=[1e6,1e4,0,f0]
#    def searchMode(self,)    
    def searchMode(self,pwr,IF0=1000,points=1001,span0=2e6,avg=1,MaxTimes=1,search_factor=5,sweep=True):
...
#                        swp_time=1.1*self.points/IF0*self.average
    ...
    def optimizeScan(self,span_factor=10):
        self.span_3db=3*self.f0/self.Qi
...
#                self.average=1
            ...
            if IF < 100:
                segment=[(self.f0-self.span_whole,self.f0-self.span_broaden,IF,N1),\
                    ...
                    (self.f0-self.span_3db,self.f0+self.span_3db,1,N3),\
...
#                self.average=1
            ...
            if IF < 10:
                IF=10
                segment=[(self.f0-self.span_whole,self.f0-self.span_broaden,10,N1),\
                    (self.f0-self.span_broaden,self.f0+self.span_broaden,1,N2),\
                    (self.f0+self.span_broaden,self.f0+self.span_whole,10,N1)]
            else:
                segment=[(self.f0-self.span_whole,self.f0-self.span_broaden,IF,N1),\
...
#            if self.average>1:
#                wait*=0.95
            print("Time seconds need wait:",wait)
            self.sweep()
            time.sleep(wait)
            self.fetchData()
#            self.update2(self.P,'UCSB')
            self.update2(self.P,'WANG')
            if self.q_succeed:
                print('Succeed! Power={}dBm, Qi={}, Qc={}, f0={} Hz.'.format(self.power,self.Qi,self.Qc,self.f0))
                return True
