...
Qmod.createWindow((('kaiser_r',0,N_rise*2,beta),\
                   ('rectangle',N_rise*2,N_rise*2+N_stim*2),\
                   ('kaiser_f',N_rise*2+N_stim*2,N_rise*4+N_stim*2,beta)\
                   ))
...
Ireson.createWindow((\
                     ('rectangle',N_rise*2+N_stim+N_shift,N_rise*2+N_stim+N_shift+N_hold),\
                     ))
Qreson.createWindow((\
                     ('rectangle',N_rise*2+N_stim+N_shift,N_rise*2+N_stim+N_shift+N_hold),\
                     ))
...
dg1.delayAB=t_r*2+t_stim+tao+0.4e-6
#dg1.delayAB=1e-6
#%%
#-------------波形写入DA卡-------------------------------------------#
da1.stop()
...
#%%
#-------------这一段代码用于搜索qubit频率。-------------------------------------------#
fr=6.7187e9         #设置读出腔激发微波频率，把IQ mixer造成的偏移量也算进去了。
mw2.freqency=fr

avg=2
#mw1.level=0
fq=np.linspace(6.45,6.55,101)*1e9
data=np.zeros(fq.size).astype('complex')
idx=np.abs(np.fft.fftfreq(ad1.recordlength,d=1/ad1.samplerate)+f_b).argmin()            #计算出信号出现的频率点。
...
#    sleep(1e-3)
    for k in range(avg):
        ...
        data[i]=(data[i]*k+Dfft)/(k+1)
    np.savez('c5q5_sweep_fq',data=data,fr=fr,fq=fq) 
    print(i)
#plot(data[0])
figure(),plot(fq,np.abs(data))
#np.savez('temp3',data=data,fr=fr,fq=fq)    
#%%
fq=6.4822e9
mw1.freqency=fq
fr=np.linspace(6.714,6.725,51)*1e9
data=np.zeros(fr.size).astype('complex')
...
figure(),plot(fr,rf.mag_2_db(np.abs(data)))
#%%
#cal_data=data
data=np.zeros(fr.size).astype('complex')
for i in range(fr.size):
    ...
    ad1.start()
    d=ad1.data
    data[i]=d[0].mean()+1j*d[1].mean()
figure(),plot(fr,rf.mag_2_db(np.abs(data)))
figure(),plot(fr,rf.mag_2_db(np.abs(data-cal_data)))
#%%
fq=6.483e9
fr=6.7187e9
mw1.freqency=fq
mw2.freqency=fr
tao=np.linspace(0,5e-6,101)
...
#        T1data[i]=(T1data[i]*k+Ifft2[8]+1j*Qfft2[8])/(k+1)
    np.savez('c5q5_T1_3',T1data=T1data,tao=tao,fr=fr,fq=fq) 
    print(i)
