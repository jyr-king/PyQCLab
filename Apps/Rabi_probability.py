...
#%%
#-------------加X方向微波的Rabi。-------------------------------------------#
t_stim=np.linspace(0,0.5e-6,51)
RabidataX=np.zeros(t_stim.size)
RabidataX2=np.zeros(t_stim.size).astype('complex')
areaX=np.zeros(t_stim.size)
#t_meas=3e-6
#t_r=10e-9
#N_hold=int(t_meas*da2.sampleRate)
#N_rise=int(t_r*da2.sampleRate)
...
for i in range(t_stim.size):
    ...
    Imod.createWaveform()
    Ioff.createWaveform()
    areaX[i]=integrate.simps(Imod.rawWaveform,dx=1/da1.sampleRate)
...
#        S[k]=Dfft_n.real > 0
        ...
        RabidataX2[i]=(RabidataX2[i]*k+Dfft.mean())/(k+1)
...
#        T1data[i]=(T1data[i]*k+Ifft2[8]+1j*Qfft2[8])/(k+1)
    np.savez('c5q5_Rabi_probabilityX-3',RabidataX=RabidataX,RabidataX2=RabidataX2,t_stim=t_stim,areaX=areaX,fr=fr,fq=fq) 
    print(i,RabidataX[i])
plot(areaX,RabidataX)
#%%
#-------------加Y方向微波的Rabi。-------------------------------------------#
#t_stim=np.linspace(0,1e-6,201)
RabidataY=np.zeros(t_stim.size)
RabidataY2=np.zeros(t_stim.size).astype('complex')
areaY=np.zeros(t_stim.size)
...
for i in range(t_stim.size):
    N_stim=int(t_stim[i]*da2.sampleRate)
    Qmod.createWindow((('kaiser_r',N_t1*2-N_shift*2-N_rise*4-N_stim*2,N_t1*2-N_shift*2-N_rise*2-N_stim*2,beta),\
                   ('rectangle',N_t1*2-N_shift*2-N_rise*2-N_stim*2,N_t1*2-N_shift*2-N_rise*2),\
                   ('kaiser_f',N_t1*2-N_shift*2-N_rise*2,N_t1*2-N_shift*2,beta)\
                   ))
    Ioff.createWindow((('rectangle',N_t1-N_shift-N_stim-N_rise*2,N_t1-N_shift),\
    ...
    Qmod.createWaveform()
    areaY[i]=integrate.simps(Qmod.rawWaveform,dx=1/da1.sampleRate)
    Ioff.createWaveform()
...
#        S[k]=Dfft_n.real > 0
        ...
        Dfft_n=(Dfft-center)*np.exp(-1j*angle)
        S[k]=(Dfft_n.real > 0).sum()
        RabidataY2[i]=(RabidataY2[i]*k+Dfft.mean())/(k+1)
#    S=S*np.exp(1j*2*np.pi*f_b*tao[i])
#    plot(S.real,S.imag)
    RabidataY[i]=S.sum()
#        ad1.start()
...
#        Ifft2,Qfft2=np.abs(np.fft.rfft(I2)),np.abs(np.fft.rfft(Q2))
#        T1data[i]=(T1data[i]*k+Ifft2[8]+1j*Qfft2[8])/(k+1)
    np.savez('c5q5_Rabi_probabilityY-2',RabidataY=RabidataY,RabidataY2=RabidataY2,t_stim=t_stim,areaY=areaY,fr=fr,fq=fq) 
    print(i,RabidataY[i])
plot(areaY,RabidataY)
#%%
#-------------寻找准确的pi/2脉冲和pi脉冲。-------------------------------------------#
N_window=np.arange(20,500,10)
#t_stim=np.linspace(0,0.5e-6,51)
RabidataX=np.zeros(N_window.size)
RabidataX2=np.zeros(N_window.size).astype('complex')
areaX=np.zeros(N_window.size)
#t_meas=3e-6
#t_r=10e-9
#N_hold=int(t_meas*da2.sampleRate)
#N_rise=int(t_r*da2.sampleRate)
...
idx=np.abs(np.fft.fftfreq(ad1.recordlength,d=1/ad1.samplerate)+f_b).argmin()
figure()
for i in range(N_window.size):
#    N_stim=int(t_stim[i]*da2.sampleRate)
    Imod.createWindow((\
                   ('kaiser',N_t1*2-N_window[i],N_t1*2,12),\
                   
                   ))
    Ioff.createWindow((('rectangle',N_t1-math.ceil(N_window[i]/2),N_t1),\
                   ))
...
#        Ifft2,Qfft2=np.abs(np.fft.rfft(I2)),np.abs(np.fft.rfft(Q2))
#        T1data[i]=(T1data[i]*k+Ifft2[8]+1j*Qfft2[8])/(k+1)
    np.savez('c5q5_Rabi_probabilityX-5',RabidataX=RabidataX,RabidataX2=RabidataX2,t_stim=t_stim,areaX=areaX,fr=fr,fq=fq) 
    print(i,RabidataX[i])
plot(areaX,RabidataX)