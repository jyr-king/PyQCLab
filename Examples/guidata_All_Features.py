...
class SubDataSet(DataSet):
    ...
    fname = FileOpenItem("Single file (open)", ("csv", "eta"), TEMPDIR+'h.csv')
    fnames = FilesOpenItem("Multiple files", "csv", TEMPDIR+'h.csv')
    fname_s = FileSaveItem("Single file (save)", "eta", TEMPDIR+'\\h')
...
class TestParameters(DataSet):
    ...
    floatarray = FloatArrayItem("Float array", default=np.ones( (50, 5), float),
                                format=" %.2e ").set_pos(col=1,colspan=2)
